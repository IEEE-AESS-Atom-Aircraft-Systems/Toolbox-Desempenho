%Tração Disponível x Tração Requerida 
...
if (isempty (dims))
  helpdlg ('Preencha corretamente', 'Trações Disponível e Requerida');
else
...
%Polar de arrasto para perfil arqueado (cumbered)
  ...
  grid on
  plot (V,Tr,'-b');
  title('Tração Disponível x Tração Requerida');
  xlabel('Velocidade do escoamento livre V (m/s)');
  ylabel('Tração (N)');
  ylim([-2 , 220])
  xlim([-2 , 32])
  legend('Tração Disponível', 'Tração Requerida');
  
  helpdlg (sprintf ('O gráfico das Trações Disponível e Requerida foi plotado.'), 'Trações Disponível e Requerida');
endif
