%Potência Disponível x Potência Requerida (difentes altitudes)
...
if (isempty (dims))
  helpdlg ('Preencha corretamente', 'Potência em altitute');
else
  ...
  grid on
  plot (v,Pd,'-r');
  title('Potência Disponível em diferentes altitudes');
...
%Polar de arrasto para perfil arqueado (cumbered)
  ...
  Pr = Tr .* V;
  plot(V, Pr, '-r');
  title('Potência Requerida em diferentes altitudes');
  xlabel('Velocidade do escoamento livre V (m/s)');
  ylabel('Potência (W)');
  ylim([0, 60])
  xlim([0 , 30])
  ...
  helpdlg (sprintf ('A densidade do ar na altura de operação do VANT corresponde a %.4f kg/m³.',...
                     rho_operacao), 'Potência em altitute');
endif
